...
function Je(o = localStorage) {
    try {
        const t = o.getItem(Xe);
        return t && Nt(t) ? Ye(t) : void 0;
    } catch {
        return;
    }
}
...
function zn(o, t) {
    ...
    if (t.type === "refresh") return Dn(o, t.items);
...
function An(o, t, e) {
    const n = Math.max(1, Math.ceil(o.length / De)), r = Math.max(0, Math.min(t, n - 1)), i = r * De, u = o.slice(i, i + De).map((s, f) => {
        var x;
        const d = i + f, l = ((x = s.title) == null ? void 0 : x.trim()) || e.untitled, c = s.read ? e.read : e.unread;
        return {
            kind: "item",
            itemIndex: d,
            label: Bn(`${d + 1}. [${c}] ${l}`)
...
function YnR1(o) {
    ...
    if (n === void 0 && (r.includes("click_event") || r.includes("tap") || r.includes("ring") || r.includes("touch"))) n = M.CLICK_EVENT;
    if (n === void 0) return;
...
function Rn(o) {
    return o.some(t => t.toLowerCase().split("-")[0] === "da") ? "da" : "en";
}
...
function _e(o, t) {
    ...
    const r = qe(o), i = Math.min(o.pageIndex, r.length - 1), u = r[i] ?? "", s = ((h = n.title) == null ? void 0 : h.trim()) || e.untitled, f = n.type === "url" ? e.typeUrl : e.typeText, d = n.read ? e.readStateRead : e.readStateUnread, l = e.readerMeta(o.selectedIndex + 1, o.items.length, i + 1, r.length, `${f} - ${d}`), c = e.readerHelp, x = `${l}\n${s}\n\n${u}\n\n${c}`;
    return {
        status: "ready",
...
function G2HomeCardText(o, t, e = !1) {
    if (!o) return "";
    // Allow the selected card to wrap to two short lines on G2 instead of
    // truncating almost everything. Side cards stay shorter to avoid overflow.
    return G2HomeClamp(o.label, e ? 40 : 26);
}
function G2HomeTime() {
    try {
...
async function G2HomeUpgrade(o, t, e, n = 0) {
    ...
    for (const [u, s, f] of i) await o.textContainerUpgrade(new ue({
        containerID: u,
        containerName: s,
        contentOffset: 0,
        contentLength: 1000,
...
async function Qn() {
    ...
    const t = Rn(navigator.languages), e = Ze(t), n = new URLSearchParams(window.location.search), r = n.get("demo") === "1";
    ...
    let i = {
        status: "loading"
    }, u, s, f = "menu", d = "text", l = "", c = 0, m = 0, x = 0, h, p = !1;
    const w = new Tn, y = (C, N) => {
        ...
        if (nr(i) && f === "menu") {
            ...
            Number.isInteger(J) ? m = Math.max(0, Math.min(K.entries.length - 1, J)) : K.itemCount > 0 && (m = Math.max(0, K.entries.findIndex(te => te.kind === "item")));
            ...
            if (ve === l) return;
            if (d === "home" && globalThis.__sendToG2HomeSideSig === g2HomeSideSig) {
                await G2HomeUpgrade(u, K, e, m);
                l = ve;
                return;
            }
        ...
        C.kind === "item" && (f = "delete-confirmation", await g({
            type: "select-item",
            index: C.itemIndex
        }));
    }, _ = async () => {
        ...
        try {
            ...
            const N = xe(C);
            return N === "unauthorized" ? (await g({
                type: "fail",
                reason: N
            }), "terminal-failure") : (p || await g({
                type: "fail",
                reason: N
            }), "retryable-failure");
        }
    ...
    r ? await g({
        type: "load",
        items: ht
    }) : (s = new Et(void 0, void 0, void 0, Je()), h = new On(le, void 0, void 0, void 0, Jn), 
...
async function $n(o, t, e, n) {
    ...
    s = new Et(void 0, void 0, void 0, Je());
    try {
...
function ur(o, t, e) {
    ...
    t.items.forEach((i, u) => {
        ...
        s.type = "button", s.className = "item-picker-button", s.dataset.selectItemIndex = String(u), 
        s.setAttribute("aria-current", u === t.selectedIndex ? "true" : "false");
        const f = ((l = i.title) == null ? void 0 : l.trim()) || e.untitled, d = i.read ? e.readStateRead : e.readStateUnread;
        s.textContent = `${u + 1}. ${f} - ${d}`, r.append(s);
    })));
}
...
function fr(o, t, e) {
    const n = new Pn, r = o.onEvenHubEvent(i => {
        ...
        u.kind === "click" ? t() : u.kind === "double-click" ? e() : u.kind === "exit" && r();
    });
}
